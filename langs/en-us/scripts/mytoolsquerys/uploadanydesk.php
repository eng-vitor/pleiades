...
if(strlen($IdAnyDeskCreate)>=9 and strlen($IdAnyDeskCreate)<=12 and strlen($PassAnyDeskCreate)>=8 and strlen($PassAnyDeskCreate)<=32){
    ...
    /* Format Tools*/
    $QueryIfExists = "SELECT login,pass FROM usertools WHERE tipotool=1 AND id_conta='".$_SESSION['DataAccount']['id']."'";
    ...
    if($QueryIfExistsRow==0){
        $QueryRegisterAnydesk = "INSERT INTO usertools(id,id_conta,tipotool,login,pass) VALUES ('','".$_SESSION['DataAccount']['id']."','1','$IdAnyDeskCreate','$PassAnyDeskCreate')";
        mysqli_query($CONNECTION_DB, $QueryRegisterAnydesk);
        ...
        $QueryRegisterAnydesk = "UPDATE usertools SET login='$IdAnyDeskCreate',pass='$PassAnyDeskCreate' WHERE tipotool=1 AND id_conta='".$_SESSION['DataAccount']['id']."'";
        mysqli_query($CONNECTION_DB, $QueryRegisterAnydesk);
