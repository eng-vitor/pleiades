...
if(isset($_SESSION['DataAccount']['id'])){
    $_SESSION['Msg'] = '<div class="alert alert-warning" role="alert"><i class="fa-solid fa-triangle-exclamation fa-beat"></i> You are already logged in!</div>';
...
if($QueryEmailExistsResult){
	...
	if ($QueryAccountDataResult){
		...
		$_SESSION['Msg'] = '<div class="alert alert-danger" role="alert" style="width: 100%"><i class="fa-solid fa-xmark fa-beat"></i> The entered password is incorrect!</div>';
		header("Location: ../index.php");
		exit();
	}
}else{
	$_SESSION['Msg'] = '<div class="alert alert-warning" role="alert" style="width: 100%"><i class="fa-solid fa-exclamation fa-beat"></i> The email entered is not in the database!</div>';
	header("Location: ../index.php");
	exit();
}
