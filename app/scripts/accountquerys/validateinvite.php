...
if($ValidateInvite!=' ' and !strpbrk($ValidateInvite,'!@#$%¨&*()-_+={}[]')){
    ...
    if(mysqli_num_rows($InviteListExec)==1){
        ...
        $_SESSION['Msg'] = '<div class="alert alert-warning" role="alert" style="width: 100%"><i class="fa-solid fa-triangle-exclamation fa-beat"></i> O código de convite não existe ou já foi utilizado!</div>';
        header("Location: ../../pages/Create_Account.php");
        exit();
    }
}
else{
    $_SESSION['Msg'] = '<div class="alert alert-danger" role="alert" style="width: 100%"><i class="fa-solid fa-circle-exclamation fa-beat"></i> Código de convite inválido!</div>';
    header("Location: ../../pages/Create_Account.php");
    exit();
}
