...
if(strlen($ChangeProfileUsername)>=3 and strlen($ChangeProfileUsername)<100 and strlen($ChangeProfileSocial)>=4 and strlen($ChangeProfileSocial)<50 and strlen($ChangeProfileEmail)>=5 and strlen($ChangeProfileEmail)<100 and strlen($ChangeProfileUrl)>=5 and strlen($ChangeProfileUrl)<100){
    ...
    if($ChangeProfileSocial==$_SESSION['DataAccount']['social'] and $ChangeProfileEmail==$_SESSION['DataAccount']['email']){
        ...
        $_SESSION['Msg'] = '<div class="alert alert-success" role="alert"><i class="fa-solid fa-circle-check fa-beat"></i> Informações foram atualizadas no banco de dados!</div>';
        ...
        $_SESSION['Msg'] = '<div class="alert alert-success" role="alert"><i class="fa-solid fa-circle-check fa-beat"></i> Informações foram atualizadas com sucesso no banco de dados!</div>';
        header("Location: ../pages/MyProfile.php");
        exit();
        ...
        if($QuerySocialExistsResult==0){
            ...
            $_SESSION['Msg'] = '<div class="alert alert-warning" role="alert"><i class="fa-solid fa-triangle-exclamation fa-beat"></i> O nome de usuário já existe em nosso banco de dados, por favor tente outro!</div>';
            header("Location: ../pages/MyProfile.php");
            exit();
        }
        ...
        if($QueryDataExistsResult==0){
            ...
            $_SESSION['Msg'] = '<div class="alert alert-warning" role="alert"><i class="fa-solid fa-triangle-exclamation fa-beat"></i> O nome de usuário ou e-mail já existe em nosso banco de dados, por favor tente outro!</div>';
            header("Location: ../pages/MyProfile.php");
            exit();
        }
    }
}else{
    $_SESSION['Msg'] = '<div class="alert alert-danger" role="alert"><i class="fa-solid fa-circle-xmark fa-beat"></i> As condições não foram atendidas! Por favor, tente novamente.</div>';
    header("Location: ../pages/MyProfile.php");
	exit();
}
