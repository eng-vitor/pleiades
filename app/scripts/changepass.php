...
if($ChangeNewPass==$ChangeNewPassConfirm and strlen($ChangeNewPass)>=8 and strlen($ChangeNewPass)<=32 and strpbrk($ChangeNewPass,'!@#$%¨&*()')){
        ...
        if($_SESSION['DataAccount']['senha']!=$SenhaEncrypt){
            ...
            $_SESSION['Msg'] = '<div class="alert alert-success" role="alert"><i class="fa-solid fa-circle-check fa-beat"></i> As condições foram atendidas e a senha foi trocada!</div>';
            header("Location: ../pages/Change_Password.php");
            exit();
        }
        else{
            $_SESSION['Msg'] = '<div class="alert alert-warning" role="alert"><i class="fa-solid fa-triangle-exclamation fa-beat"></i> A nova senha não pode ser igual a atual!</div>';
            header("Location: ../pages/Change_Password.php");
	        exit();
        }
}
